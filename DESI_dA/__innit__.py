...
class DESI_dA(Likelihood):
    ...
    def loglkl(self, cosmo, data):
        ...
        H_arr = np.array([])
        for z in z_arr: 
            dA = self.cosmo.angular_distance(z)/self.cosmo.rs_drag()
            H = self.cosmo.Hubble(z)*self.cosmo.rs_drag()
            np.append(dA_arr, dA)
            np.append(H_arr, H)  
        # Order is important dA before H
        theory = np.append(dA_arr, H_arr)
        inv_cov_data = np.linalg.inv(self.cov_data)
        chi2 = np.dot(np.dot(np.transpose(self.data-t), inv_cov_data), self.data-t) 
